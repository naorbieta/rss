...
12 条件を、実装箇所とローカル確認に対応づけます。推薦判断は実装対象外で、README に境界を記載しています。
...
| 9. 推薦ロジックを入れない | `src/index.ts` に推薦処理なし | README の責務境界 |
| 10. 推薦理由を入れない | 同上 | README の ChatGPT 依頼例 |
| 11. 0 件判断を入れない | 同上 | README の責務境界 |
